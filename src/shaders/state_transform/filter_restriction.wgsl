...
@group(0) @binding(0) var initial_board: texture_storage_2d<u32>;
@group(0) @binding(1) var restriction:   texture_storage_2d<u32>;
...
{
    let initial_board_unrestricted: u32 = textureLoad(global_thread_id.xy, vec2<i32>(global_thread_id.xy), 0).x;
    let restriction_mask:           u32 = textureLoad(global_thread_id.xy, vec2<i32>(global_thread_id.xy), 0).x;
