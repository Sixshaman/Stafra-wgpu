...
fn main(@builtin(global_invocation_id) global_thread_id: vec3<u32>)
{
    ...
    let packed_restriction = 0xffffffff;
	...
	let on_right_bottom: vec2<bool> = (global_thread_id.xy == out_restriction);

	let right_bottom_mask = vec2<u32>(right_quad_mask, bottom_quad_mask) * vec2<u32>(on_right_bottom);
    textureStore(out_initial_stability, vec2<i32>(global_thread_id.xy), vec4<u32>(packed_restriction & ~(right_bottom_mask.x | right_bottom_mask.y)));
}